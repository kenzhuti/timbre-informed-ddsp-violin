...
class VoiceAutoEncoder(pl.LightningModule):
    def __init__(
        ...
        super().__init__()
        print("use_hmag_embedding:", use_hmag_embedding, "use_inharmonicity:", use_inharmonicity)

        #hmag
        self.use_hmag_embedding = use_hmag_embedding
        self.use_inharmonicity = use_inharmonicity

        # pass the hmag&inharmonicity parameter to the encoder
        encoder_init_args["use_hmag_embedding"] = use_hmag_embedding
        encoder_init_args["use_inharmonicity"] = use_inharmonicity
    ...
    def forward(
        ...
        x: AudioTensor = None,
        f0: AudioTensor = None,
        hmag_labels: torch.Tensor = None,  # New parameter
        ...
        if x is not None:
            ...
            encoder_kwargs = {"f0": f0}
            if self.use_hmag_embedding and hmag_labels is not None:
                # print("ae.py forward() using use_hmag_embedding", self.use_hmag_embedding)
                encoder_kwargs["hmag_labels"] = hmag_labels
            enc_params: Dict = self.encoder(x, **encoder_kwargs)
            params.update(enc_params)
    ...
    def training_step(self, batch, batch_idx):
        # Handle batches of different lengths
        if len(batch) == 3:
            x, f0_in_hz, hmag_labels = batch
        else:  # len(batch) == 2, compatible with cases that don't return hmag
            x, f0_in_hz = batch
            hmag_labels = None
        ...
        encoder_kwargs =  {"f0": f0_in_hz if self.train_with_true_f0 else None}
        if self.use_hmag_embedding and hmag_labels is not None:
            # print("ae.py training_step() using use_hmag_embedding", self.use_hmag_embedding)
            encoder_kwargs["hmag_labels"] = hmag_labels
        params: Dict = self.encoder(x, **encoder_kwargs)  # f0=f0_in_hz if self.train_with_true_f0 else None
        f0_hat = params.pop("f0", None)
        
        #hmag_logits = params.pop("hmag_logits", None)
        #hmag_embeddings = params.pop("hmag_embeddings", None)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.train_with_true_f0:
            phase = torch.where(f0_in_hz == 0, 150, f0_in_hz) / self.sample_rate
            x_hat, enc_params = self(x, f0_in_hz,hmag_labels, {"phase": phase})  # Use true F0 to evaluate upper bound → need to pass params. Use predicted F0 to evaluate actual performance → no need to pass params.
        else:
            x_hat, enc_params = self(x,timber_cluster=hmag_labels)
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.train_with_true_f0:
            phase = torch.where(f0_in_hz == 0, 150, f0_in_hz) / self.sample_rate
            x_hat, enc_params = self(x, f0_in_hz,hmag_labels, {"phase": phase})
        else:
            x_hat, enc_params = self(x,timber_cluster=hmag_labels)
        loss = self.criterion(x_hat[:, : x.shape[1]], x[:, : x_hat.shape[1]]).item()
        N = x_hat.shape[0]
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=None):     
        # x, f0_in_hz, hmag_labels,rel_path = batch
        # Handle batches of different lengths
        if len(batch) == 4:
            x, f0_in_hz, hmag_labels, rel_path = batch
        else:  # len(batch) == 3
