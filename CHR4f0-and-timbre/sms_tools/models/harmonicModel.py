...
#    print("hfreqp: ", hfreqp)
    #if hfreqp == [] or np.array_equal(hfreqp, np.zeros(nH)):  # if no incomming harmonic tracks initialize to harmonic series
    if len(hfreqp) == 0:
        hfreqp = hf
    while (f0 > 0) and (hi < nH) and (hf[hi] < fs / 2) and (len(pfreq)>0):  # find harmonic peaks
