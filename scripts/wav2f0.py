...
if __name__ == "__main__":
    ...
    parser.add_argument("--method", choices=["dio", "penn", "swipe"], default="swipe")
    ...
    # for Violin: Lowest note (G3): ~196 Hz (the open G string).Highest note (E7): ~3,520 Hz (on the E string,
    parser.add_argument("--f0-floor", type=float, default=150.0, help="F0 floor in Hz")
    parser.add_argument("--f0-ceil", type=float, default=4000.0, help="F0 ceil in Hz")
    
    args = parser.parse_args()

    print(f"Extracting f0 from {args.wav_dir} using method {args.method}")
    print(f"F0 floor: {args.f0_floor} Hz, F0 ceil: {args.f0_ceil} Hz")
